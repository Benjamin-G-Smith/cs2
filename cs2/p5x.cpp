...
// PURPOSE: Allows a game of stack 13 to be played between two to 6 players.
...
//  Also if the players would like to run the game again. The number of
//  players.
...
int main()
{
  ...
  cin >> playGame;
  cout << "Enter the number of player [ 2-6 ]: ";
  cin >> PLAYER_NUM;
  cin.get();
  while(PLAYER_NUM > 6 || PLAYER_NUM < 2){
    cout << "Invalid player entry..."<< endl;
    cout << "Enter the number of player [ 2-6 ]: ";
    cin >> PLAYER_NUM;
  }
  ...
  while(toupper(playGame) == 'Y'){
    ...
    for(int i = 0; i < 7; i++){ // deals 7 cards to each player
      int p = 0;
      while(p < PLAYER_NUM){
        drawCard(players[p].hand,cards,deckCount);
        p++;
      }
    }
    playerNum = 0;
